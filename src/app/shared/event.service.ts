...
export class EventService {
  ...
  getEvents(): IEvent[]//Observable<IEvent[]>
  {
    let url = '../assets/event-data.js';
    return this.EVENTS;
    // return this._http.get(url).pipe(map(response=> <IEvent[]>response));
    // return this._http.get(url).pipe(map(response=> <IEvent[]>response.json()));
  }
